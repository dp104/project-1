...
import {
    ...
    IndianRupee, Search, X, Tag, Truck, Plus
} from 'lucide-react'
...
export function AdminPanel() {
    ...
    const toggleFeatured = p => supabase.from('products').update({ is_featured: !p.is_featured }).eq('id', p.id).then(fetchProducts)
    const toggleActive = p => supabase.from('products').update({ is_active: !p.is_active }).eq('id', p.id).then(fetchProducts)
